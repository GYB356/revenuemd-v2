...
export function MedicalRecordDialog({
  ...
  const onSubmit = async (data: MedicalRecordFormData) => {
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to save medical record",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
    ...
    <Dialog open={isOpen} onOpenChange={() => onClose(false)}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            View and update patient medical record information
          </DialogDescription>
        </DialogHeader>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <AttachmentUploader
                        ...
                        onUploadComplete={() => loadMedicalRecord(patientId)}
                      />
                      <FormMessage />
                    </FormItem>
                  )}
